  ...
  tools: [
    {
      ...
      async execute({ prompt, aspectRatio, quality }: { prompt: string; aspectRatio: string; quality: string }) {
        ...
        try {
          ...
          if (!hasApiKey) {
            logger.info({ prompt }, 'Triggering simulated Nano Banana image generation (no valid Gemini API key found)...');
            return {
              success: true,
              ...
              instructions: 'Configure a valid Gemini API key in adytum.config.yaml to enable real image generation.'
            };
          }

          const baseUrl = googleConfig?.baseUrl || 'https://generativelanguage.googleapis.com/v1beta';
          const model = 'gemini-2.5-flash-image';
          const apiUrl = `${baseUrl}/models/${model}:generateContent?key=${apiKey}`;
          
          logger.info({ model, prompt }, 'Triggering real Nano Banana image generation via Gemini API...');
