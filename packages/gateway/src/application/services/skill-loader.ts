...
const resolveMetadata = (data: Record<string, unknown>) => {
  ...
  const requires =
    (data.requires as Record<string, unknown>) || (oc.requires as Record<string, unknown>) || {};
  const primaryEnv =
    typeof (data.primaryEnv || oc.primaryEnv) === 'string'
  ...
  const skillKey =
    typeof (oc.skillKey ?? rawMeta.skillKey) === 'string'
      ? String(oc.skillKey ?? rawMeta.skillKey).trim()
      : undefined;
  const homepage =
    typeof (oc.homepage ?? rawMeta.homepage) === 'string'
      ? String(oc.homepage ?? rawMeta.homepage).trim()
      : undefined;
  const emoji =
    typeof (oc.emoji ?? rawMeta.emoji) === 'string'
...
export class SkillLoader {
  ...
  async init(toolRegistry: ToolRegistry): Promise<void> {
    ...
    for (const skill of enabled) {
      ...
      try {
        const raw = (await this.jiti.import(skill.source)) as
          | SkillModuleExport
          | { default: SkillModuleExport };
        const normalized = this.resolveSkillModule(raw);

        if (!normalized) {
          throw new Error(
            'Skill must export a plugin definition, register function, or legacy Adytum skill object',
          );
        }
    ...
    for (let i = this.services.length - 1; i >= 0; i -= 1) {
      ...
      try {
        ...
        registration.logger.warn(
          `service stop failed (${registration.service.id}): ${err?.message || err}`,
        );
      }
    }
  ...
  getSkillsContext(): string {
    ...
    for (const skill of active) {
      ...
      // Inject configuration context so the agent knows about its settings (e.g. Discord IDs)
  ...
  private discoverCandidates(): SkillCandidate[] {
    const buckets: Array<{ roots: string[]; origin: SkillOrigin; priority: number }> = [
      ...
      {
        roots: this.managedSkillsDir ? [this.managedSkillsDir] : [],
        origin: 'managed',
        priority: 2,
      },
      {
  ...
  private resolveEntrySource(rootDir: string): string | undefined {
    ...
    if (existsSync(packageJsonPath)) {
      try {
        ...
        const extensions = Array.isArray(adytum?.extensions) ? adytum?.extensions : undefined;
  ...
  private resolveEnableState(
    ...
    for (const envName of requiredEnv) {
      ...
      if (
        !configured ||
        (metadata.primaryEnv && envName !== metadata.primaryEnv && !entry?.env?.[envName])
      ) {
  ...
  private applyEnvOverrides(skill: LoadedSkill) {
    ...
    if (skill.id === 'discord') {
      ...
      if (token && !process.env.ADYTUM_DISCORD_BOT_TOKEN)
        process.env.ADYTUM_DISCORD_BOT_TOKEN = token;
      if (chan && !process.env.ADYTUM_DISCORD_DEFAULT_CHANNEL_ID)
        process.env.ADYTUM_DISCORD_DEFAULT_CHANNEL_ID = chan;
      if (user && !process.env.ADYTUM_DISCORD_DEFAULT_USER_ID)
        process.env.ADYTUM_DISCORD_DEFAULT_USER_ID = user;
      if (cfgToken && !process.env.ADYTUM_DISCORD_BOT_TOKEN)
        process.env.ADYTUM_DISCORD_BOT_TOKEN = cfgToken;
      if (cfgChan && !process.env.ADYTUM_DISCORD_DEFAULT_CHANNEL_ID)
        process.env.ADYTUM_DISCORD_DEFAULT_CHANNEL_ID = cfgChan;
      if (cfgUser && !process.env.ADYTUM_DISCORD_DEFAULT_USER_ID)
        process.env.ADYTUM_DISCORD_DEFAULT_USER_ID = cfgUser;
      if (cfgGuild && !process.env.ADYTUM_DISCORD_GUILD_ID)
        process.env.ADYTUM_DISCORD_GUILD_ID = cfgGuild;
    }
  }
  ...
  private resolveSkillModule(
    ...
    | null {
    const resolved =
      isRecord(rawModule) && 'default' in rawModule
    ...
    const hasPluginShape =
      typeof resolved.register === 'function' || typeof resolved.activate === 'function';
  ...
  private async loadPluginSkill(
    ...
    if (definition.id && definition.id !== skill.id) {
      logger.warn(
        `plugin id "${definition.id}" does not match manifest id "${skill.id}"; manifest id is used for config lookup`,
      );
    }
    ...
    const api: AdytumSkillPluginApi = {
      ...
      resolvePath: (value: string) =>
        isAbsolute(value) ? resolve(value) : resolve(skill.path, value),
