...
export class GraphIndexer {
  private supportedExtensions = [
    '.ts', '.js', '.tsx', '.jsx', '.py', '.go', '.md', '.yaml', '.yml', '.dart',
    '.pdf', '.zip', '.png', '.jpg', '.jpeg', '.txt', '.json'
  ];
  private ignoredDirs = [
    'node_modules', '.git', 'dist', 'build', '.dart_tool', 
    'ios', 'android', 'macos', 'windows', 'linux',
    '.fvm', '.symlinks', '.cxx', 'Pods', 'Carthage', 
    'Debug', 'Release', 'derivedData', '.adytum_extracted'
  ];
  ...
  async update(
    ...
    logger.info(`Scan [${options.mode}] completed in ${Date.now() - startTime}ms. Total nodes: ${graph.nodes.length}, Edges: ${graph.edges.length}`);
    return graph;
  }

  private ensureDirectoryNodes(relPath: string, nodes: GraphNode[], edges: GraphEdge[], workspacePath: string): void {
  ...
  private extractCodeRelationships(filePath: string, node: GraphNode, graph: KnowledgeGraph, workspacePath: string): void {
      ...
      try {
          ...
          logger.warn(`Failed to extract relationships from ${filePath}: ${err instanceof Error ? err.message : err}`);
      }
  }

  private addImportEdge(sourceNode: GraphNode, targetPath: string, graph: KnowledgeGraph, sourceFullPath: string, workspacePath: string): void {
      ...
      const targetNode = graph.nodes.find(n => n.id === resolvedTarget || n.path === resolvedTarget);
