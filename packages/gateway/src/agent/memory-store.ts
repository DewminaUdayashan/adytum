...
export function redactSecrets(input: string): string {
  ...
  const patterns: Array<{ regex: RegExp; replace: string | ((match: string, key?: string) => string) }> = [
    ...
    { regex: /\b[A-Za-z0-9_\-]{20,40}\.[A-Za-z0-9_\-]{4,10}\.[A-Za-z0-9_\-]{20,120}\b/g, replace: '[REDACTED_DISCORD_TOKEN]' },
    // Clean up previously partial-redacted token artifacts
    { regex: /\b[A-Za-z0-9_\-]*\[REDACTED_DISCORD_TOKEN\][A-Za-z0-9_\-]*\b/g, replace: '[REDACTED_DISCORD_TOKEN]' },
    // OpenAI-style keys
    ...
    {
      regex: /\b(ADYTUM_DISCORD_DEFAULT_CHANNEL_ID|ADYTUM_DISCORD_GUILD_ID|ADYTUM_DISCORD_USER_ID)\b\s*[:=]\s*[`'"]?(\d{17,20})[`'"]?/gi,
    ...
    // Discord channel URLs with IDs
    {
      ...
      replace: 'discord.com/channels/[REDACTED_DISCORD_ID]/[REDACTED_DISCORD_ID]/[REDACTED_DISCORD_ID]',
    },
