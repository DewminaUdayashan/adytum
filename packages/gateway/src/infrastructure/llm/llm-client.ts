...
export class LLMClient {
   ...
   */
  async chat(modelConfig: ModelConfig, options: LLMChatOptions): Promise<LLMChatResult> {
    ...
    let piModel = await this.catalog.getPiModel(modelId);
    ...
    try {
      ...
      const result = await pi.complete(
        piModel,
        {
