...
import { Plus, Folder, FileText, Trash2, ArrowRight, HardDrive, RefreshCw, FolderOpen } from 'lucide-react';
...
export default function WorkspacesPage() {
  ...
  const [newWorkspace, setNewWorkspace] = useState<{ name: string; path: string; type: 'project' | 'collection' }>({ 
    name: '', 
    path: '', 
    type: 'project' 
  });
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this workspace? This won\'t delete your local files, but it will remove the knowledge graph.')) return;
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto animate-fade-in">
      <div className="flex items-center justify-between mb-10">
        <div>
          <p className="text-[11px] uppercase tracking-[0.2em] text-text-muted font-medium">Workspaces</p>
          <h1 className="text-3xl font-semibold text-text-primary tracking-tight mt-1">Manage Your Projects</h1>
        </div>
        <button 
        ...
        <div className="lg:col-span-1">
          <div className="bg-bg-secondary border border-border-primary rounded-2xl p-6 sticky top-8">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-text-muted uppercase tracking-wider mb-1.5 ml-1">Name</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-xs font-medium text-text-muted uppercase tracking-wider mb-1.5 ml-1">Local Path</label>
        ...
        <div className="lg:col-span-2">
          {loading && workspaces.length === 0 ? (
            <div className="space-y-4">
              {[1, 2, 3].map(i => (
                <div key={i} className="bg-bg-secondary border border-border-primary rounded-2xl p-6 animate-pulse">
            ...
            <div className="bg-bg-secondary border border-border-primary border-dashed rounded-3xl p-16 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-text-tertiary max-w-sm text-sm">
                Create your first workspace by selecting a project directory or a collection of notes to start indexing your knowledge.
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="flex-1 min-w-0 pr-4">
                    <div className="flex items-center gap-2 mb-1">
                      {ws.type === 'project' ? <Folder className="h-4 w-4 text-accent-primary" /> : <FileText className="h-4 w-4 text-warning" />}
                      <h3 className="text-lg font-semibold text-text-primary truncate">{ws.name}</h3>
                    ...
                    <div className="flex items-center gap-4 mt-4">
                       ...
                       <div className="text-center">
                         ...
                         <p className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase ${ws.lastIndexed ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning'}`}>
