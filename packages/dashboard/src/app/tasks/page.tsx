      ...
      <div className="px-8 py-6 space-y-6 overflow-y-auto">
        ...
        {showCreate && (
             <Card className="p-6 border-primary/20 bg-primary/5 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <label className="block text-xs font-medium text-text-muted mb-1">Schedule (Cron)</label>
                        <input 
                ...
                <div className="flex justify-end gap-2">
                    ...
                    <Button variant="primary" size="sm" onClick={createJob}>Create Task</Button>
                </div>
             </Card>
        )}
        ...
        {jobs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4">
            {jobs.map((job) => (
              <Card key={job.id} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-start gap-4">
                    <div className={clsx(
                        "p-3 rounded-lg border",
                        job.enabled ? "bg-green-500/10 border-green-500/20 text-green-500" : "bg-text-muted/10 border-text-muted/20 text-text-muted"
                    )}>
                    ...
                    <div>
                        ...
                        <code className="text-xs text-primary font-mono mt-1 block">{job.schedule}</code>
                ...
                <div className="flex items-center gap-2 self-end sm:self-center">
                    ...
                    >
                        {job.enabled ? <Pause className="w-4 h-4 text-text-muted" /> : <Play className="w-4 h-4 text-primary" />}
                    </Button>
                    <Button 
                        size="sm" 
                        variant="ghost" 
