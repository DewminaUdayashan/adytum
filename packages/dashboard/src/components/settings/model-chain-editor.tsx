...
import { Plus, X, ArrowRight, Save, RotateCcw, ArrowUp, ArrowDown } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
           ...
           <p className="text-xs text-text-tertiary">Define fallback chains for different agent roles. The system will try models in order.</p>
        </div>
        {modified && (
          ...
          <Card key={role} className="flex flex-col gap-4">
            <div className="flex justify-between items-start">
              <div>
                <div className="flex items-center gap-2">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-text-primary">{role}</h3>
                    <Badge variant="default" size="sm" className="font-mono text-[10px]">{chains[role]?.length || 0} models</Badge>
                </div>
            ...
            <div className="flex flex-wrap items-center gap-2 min-h-[40px] p-2 bg-bg-tertiary rounded-lg border border-border-primary/50">
                {chains[role]?.length === 0 && (
                    <span className="text-xs text-text-muted italic px-2">No models configured. Will fail if called.</span>
                )}
