...
import { Folder, File, ChevronRight, Home, ArrowLeft, X, Check, Loader2, Search } from 'lucide-react';
...
export function FilePicker({ isOpen, onClose, onSelect, title = 'Select Directory' }: FilePickerProps) {
  ...
  const fetchPath = async (path?: string) => {
    setLoading(true);
    try {
      const data = await api.get(`/api/system/browse${path ? `?p=${encodeURIComponent(path)}` : ''}`);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-bg-secondary border border-border-primary rounded-3xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[80vh] overflow-hidden animate-slide-up">
        ...
        <div className="px-6 py-5 border-b border-border-primary/50 flex items-center justify-between bg-bg-tertiary/20">
          ...
          <button onClick={onClose} className="p-2 hover:bg-bg-hover rounded-xl text-text-tertiary transition-colors">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 min-h-[300px]">
          {loading ? (
             <div className="flex items-center justify-center h-full gap-3 opacity-50">
                ...
                <span className="text-xs font-semibold uppercase tracking-widest text-text-muted">Accessing Filesystem...</span>
             </div>
                ...
                {filteredItems.map((item) => (
                  <button
                    ...
                    className={clsx(
                      ...
                      item.type === 'directory' ? "hover:bg-accent-primary/5 hover:border-accent-primary/20" : "opacity-40 cursor-not-allowed"
                    )}
                  >
                    {item.type === 'directory' ? (
        ...
        <div className="px-6 py-5 border-t border-border-primary/50 bg-bg-tertiary/10 flex items-center justify-between">
          <div className="flex-1 truncate mr-4">
             <p className="text-[10px] text-text-tertiary font-bold uppercase tracking-widest mb-1">Selected Region</p>
